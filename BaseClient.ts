...
export type RequestBodyForPathAndMethod<P extends Path, M extends SupportedMethods<P>> = M extends "GET"
...
type MultipartEdgeCases = {
  [P in Path as MultipartPathInfo<PathsRedefined[P][0]> extends never
    ? never
    : `/v2${StringifyType<PathsRedefined[P][1]>}`]: MultipartPathInfo<PathsRedefined[P][0]>;
};
...
export class BaseClient<NeverThrow extends boolean> {
  ...
  async request<RequestPath extends Path, RequestMethod extends SupportedMethods<RequestPath>>(
    path: RequestPath,
  ...
  > {
    ...
    return execute(url, method, headers, JSON.stringify(body), this.neverThrow, edgecase);
  }
}
