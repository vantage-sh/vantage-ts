...
function formatConstArray(values: string[]): string {
  if (values.length <= 6) return JSON.stringify(values).replace(/,/g, ", ");
...
async function main() {
  ...
  const constantsOutput = [
    ...
    "// Do not edit this file directly.",
    "",
    ...generatedConstants.flatMap(({ name, typeName, description, values }) => [
      `/** ${description} */`,
      `export const ${name} = ${formatConstArray(values)} as const;`,
      "",
